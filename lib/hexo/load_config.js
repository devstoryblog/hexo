...
module.exports = async ctx => {
  ...
  const isScopedThemePackage = /^(@[\w-]+)[/\\]+(hexo-theme-[\w-]+)$/.test(theme);

  // if theme name is scoped package, do not try to find it in local dir
  if (isScopedThemePackage) {
    const themeDirFromNodeModules = join(ctx.plugin_dir, theme) + sep; // base_dir/node_modules/[config.theme]/ -> (base_dir/node_modules/[@scope]/hexo-theme-[name]/)

    if (await exists(themeDirFromNodeModules)) {
